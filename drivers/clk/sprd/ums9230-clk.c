...
static SPRD_PLL_SC_GATE_CLK_FW_NAME(isppll_gate, "isppll-gate", "ext-26m",
				    0xa28, 0x1000, BIT(2), CLK_IGNORE_UNUSED,
				    0, 240);
static SPRD_PLL_SC_GATE_CLK_FW_NAME(dpll0_gate, "dpll0-gate", "ext-26m", 0xa14,
				    0x1000, BIT(2), 0, 0, 240);
static SPRD_PLL_SC_GATE_CLK_FW_NAME(lpll_gate, "lpll-gate", "ext-26m", 0xa44,
				    0x1000, BIT(2), CLK_IGNORE_UNUSED, 0, 240);
static SPRD_PLL_SC_GATE_CLK_FW_NAME(twpll_gate, "twpll-gate", "ext-26m", 0xa40,
				    0x1000, BIT(2), CLK_IGNORE_UNUSED, 0, 240);
...
static SPRD_PLL_SC_GATE_CLK_FW_NAME(rpll_gate, "rpll-gate", "ext-26m", 0xa0c,
				    0x1000, BIT(2), CLK_IGNORE_UNUSED, 0, 240);
static SPRD_PLL_SC_GATE_CLK_FW_NAME(mpll0_gate, "mpll0-gate", "ext-26m", 0x9f4,
				    0x1000, BIT(2), CLK_IGNORE_UNUSED, 0, 240);
static SPRD_PLL_SC_GATE_CLK_FW_NAME(mpll1_gate, "mpll1-gate", "ext-26m", 0x9f8,
				    0x1000, BIT(2), CLK_IGNORE_UNUSED, 0, 240);
static SPRD_PLL_SC_GATE_CLK_FW_NAME(mpll2_gate, "mpll2-gate", "ext-26m", 0x9fc,
				    0x1000, BIT(2), CLK_IGNORE_UNUSED, 0, 240);
...
static struct sprd_reset_map ums9230_pmu_apb_resets[] = {
	[RESET_PMU_APB_AUDCP_AUDDSP_SOFT_RST]	= { 0x0b98, BIT(0), 0x1000 },
	...
	[RESET_PMU_APB_MM_SOFT_RST]		= { 0x0b98, BIT(3), 0x1000 },
	[RESET_PMU_APB_PUBCP_SOFT_RST]		= { 0x0b98, BIT(11), 0x1000 },
	[RESET_PMU_APB_WTLCP_SOFT_RST]		= { 0x0b98, BIT(14), 0x1000 },
	[RESET_PMU_APB_WCN_SOFT_RST]		= { 0x0b98, BIT(20), 0x1000 },
	[RESET_PMU_APB_AUDCP_SOFT_RST]		= { 0x0b98, BIT(22), 0x1000 },
...
static SPRD_PLL_HW(dpll0, "dpll0", &dpll0_gate.common.hw, 0x4, 3,
		   itable_dpll0, f_dpll0, 240, 1000, 1000, 1, 933000000);
static CLK_FIXED_FACTOR_HW(dpll0_50m, "dpll0-50m", &dpll0.common.hw,
			   32, 1, 0);
...
static const u64 itable_mpll[8] = { 7, 1200000000ULL,
...
static SPRD_PLL_HW(mpll1, "mpll1", &mpll1_gate.common.hw, 0x54, 3,
		   itable_mpll, f_mpll, 240, 1000, 1000, 1, 1000000000);
static CLK_FIXED_FACTOR_HW(mpll1_62m5, "mpll1-62m5", &mpll1.common.hw,
			   32, 1, 0);
...
static SPRD_COMP_CLK_DATA(ap_i2c4_clk, "ap-i2c4-clk", i2c_parents,
			  0x40, 0, 2, 8, 3, 0);
static SPRD_COMP_CLK_DATA(ap_i2c5_clk, "ap-i2c5-clk", i2c_parents,
			  0x44, 0, 2, 8, 3, 0);
static SPRD_COMP_CLK_DATA(ap_i2c6_clk, "ap-i2c6-clk", i2c_parents,
			  0x48, 0, 2, 8, 3, 0);
...
static SPRD_MUX_CLK_DATA(sdio0_2x_clk, "sdio0-2x-clk", sdio_parents,
			 0x74, 0, 3, UMS9230_MUX_FLAG);
static SPRD_MUX_CLK_DATA(sdio1_2x_clk, "sdio1-2x-clk", sdio_parents,
			 0x7c, 0, 3, UMS9230_MUX_FLAG);
static SPRD_MUX_CLK_DATA(emmc_2x_clk, "emmc-2x-clk", sdio_parents,
			 0x84, 0, 3, UMS9230_MUX_FLAG);

static const struct clk_parent_data ufs_hck_parents[] = {
	{ .fw_name = "ext-26m" },
	{ .hw = &twpll_48m.hw  },
	{ .hw = &twpll_192m.hw  },
	{ .hw = &twpll_256m.hw  },
...
static SPRD_COMP_CLK_DATA(ufs_hck_clk, "ufs-hck-clk", ufs_hck_parents,
			  0x8c, 0, 3, 8, 2, 0);
...
static SPRD_MUX_CLK_DATA(ufs_tx_clk, "ufs-tx-clk", ufs_tx_rx_parents,
			 0x90, 0, 2, UMS9230_MUX_FLAG);
static SPRD_MUX_CLK_DATA(ufs_rx_clk, "ufs-rx-clk", ufs_tx_rx_parents,
			 0x94, 0, 2, UMS9230_MUX_FLAG);
static SPRD_MUX_CLK_DATA(ufs_rx_1_clk, "ufs-rx-1-clk", ufs_tx_rx_parents,
			 0x98, 0, 2, UMS9230_MUX_FLAG);
...
static SPRD_SC_GATE_CLK_FW_NAME(axi_regu_eb, "axi-regu-eb", "ext-26m",
				0x0, 0x1000, BIT(28), 0, 0);
static SPRD_SC_GATE_CLK_FW_NAME(ce_sec_eb, "ce-sec-eb", "ext-26m",
				0x0, 0x1000, BIT(30), 0, 0);
static SPRD_SC_GATE_CLK_FW_NAME(ce_pub_eb, "ce-pub-eb", "ext-26m",
				0x0, 0x1000, BIT(31), 0, 0);
static SPRD_SC_GATE_CLK_FW_NAME(ufs_eb, "ufs-eb", "ext-26m",
				0xc, 0x1000, BIT(0), 0, 0);
static SPRD_SC_GATE_CLK_FW_NAME(ufs_reg_eb, "ufs-reg-eb", "ext-26m",
				0xc, 0x1000, BIT(1), 0, 0);
static SPRD_SC_GATE_CLK_FW_NAME(ckg_eb, "ckg--eb", "ext-26m",
				0xc, 0x1000, BIT(2), CLK_IGNORE_UNUSED, 0);
...
static struct sprd_reset_map ums9230_ap_apb_resets[] = {
	...
	[RESET_AP_APB_UFS_SOFT_RST]		= { 0x0004, BIT(23), 0x1000 },
	[RESET_AP_APB_UFS_REG_SOFT_RST]		= { 0x0004, BIT(24), 0x1000 },
	[RESET_AP_APB_CKG_SOFT_RST]		= { 0x0004, BIT(25), 0x1000 },
	[RESET_AP_APB_I2C5_SOFT_RST]		= { 0x0004, BIT(26), 0x1000 },
...
/* ap ahb gates */
static SPRD_SC_GATE_CLK_HW(dsi_eb, "dsi-eb", &dsi_apb_clk.common.hw,
			   0x0, 0x1000, BIT(0), 0, 0);
static SPRD_SC_GATE_CLK_HW(dispc_eb, "dispc-eb", &dispc0_clk.common.hw,
			   0x0, 0x1000, BIT(1), 0, 0);
static SPRD_SC_GATE_CLK_HW(vsp_eb, "vsp-eb", &vsp_clk.common.hw,
			   0x0, 0x1000, BIT(2), 0, 0);
...
static struct sprd_reset_map ums9230_ap_ahb_resets[] = {
	...
	[RESET_AP_AHB_VSP_VAU_SOFT_RST]		= { 0x0004, BIT(10), 0x1000 },
...
static SPRD_SC_GATE_CLK_FW_NAME(rfti_eb, "rfti-eb", "ext-26m",
...
static SPRD_SC_GATE_CLK_FW_NAME(cssys_pub_eb, "cssys-pub-eb", "ext-26m",
				0x0, 0x1000, BIT(17), CLK_IGNORE_UNUSED, 0);
static SPRD_SC_GATE_CLK_FW_NAME(bus_transmon_en, "bus-transmon-en", "ext-26m",
				0x0, 0x1000, BIT(19), CLK_IGNORE_UNUSED, 0);
static SPRD_SC_GATE_CLK_FW_NAME(top2wcn_eb, "top2wcn-eb", "ext-26m",
...
static SPRD_SC_GATE_CLK_FW_NAME(adi_2nd_eb, "adi-2nd-eb", "ext-26m",
				0x4, 0x1000, BIT(15), CLK_IGNORE_UNUSED, 0);
...
static SPRD_SC_GATE_CLK_FW_NAME(dsi_csi_test_eb, "dsi-csi-test-eb", "ext-26m",
				0x138, 0x1000, BIT(8), CLK_IGNORE_UNUSED, 0);
...
static SPRD_SC_GATE_CLK_FW_NAME(tsen_eb, "tsen-eb", "ext-26m",
				0x138, 0x1000, BIT(13), CLK_IGNORE_UNUSED, 0);
...
static struct sprd_reset_map ums9230_aon_ahb_resets[] = {
	...
	[RESET_AON_APB_ADI_2ND_SOFT_RST]	= { 0x0010, BIT(15), 0x1000 },
...
static SPRD_MUX_CLK_DATA(top_dvfs_clk, "top-dvfs-clk", top_dvfs_parents,
			 0xd4, 0, 2, UMS9230_MUX_FLAG);

static SPRD_GATE_CLK_FW_NAME(otg_utmi, "otg-utmi", "ext-26m",
			     0xd8, BIT(16), 0, 0);
...
static SPRD_GATE_CLK_HW(gpu_core_gate, "gpu-core-gate", &gpu_core_clk.common.hw,
			0x4, BIT(0), CLK_SET_RATE_PARENT, 0);

static SPRD_COMP_CLK_DATA(gpu_mem_clk, "gpu-mem-clk", gpu_parents,
			  0x8, 4, 3, 8, 3, 0);

static SPRD_GATE_CLK_HW(gpu_mem_gate, "gpu-mem-gate", &gpu_mem_clk.common.hw,
			0x8, BIT(0), CLK_SET_RATE_PARENT, 0);

static SPRD_DIV_CLK_HW(gpu_sys_clk, "gpu-sys-clk", &gpu_eb.common.hw,
		       0xc, 4, 3, 0);

static SPRD_GATE_CLK_HW(gpu_sys_gate, "gpu-sys-gate", &gpu_sys_clk.common.hw,
			0xc, BIT(0), 0, 0);
...
static struct sprd_reset_map ums9230_gpu_apb_resets[] = {
...
/* mm gate clocks */
static SPRD_SC_GATE_CLK_HW(mm_cpp_eb, "mm-cpp-eb", &cpp_clk.common.hw,
			   0x0, 0x1000, BIT(0), 0, 0);
static SPRD_SC_GATE_CLK_HW(mm_jpg_eb, "mm-jpg-eb", &jpg_clk.common.hw,
			   0x0, 0x1000, BIT(1), 0, 0);
static SPRD_SC_GATE_CLK_HW(mm_dcam_eb, "mm-dcam-eb", &dcam_if_clk.common.hw,
			   0x0, 0x1000, BIT(2), 0, 0);
static SPRD_SC_GATE_CLK_HW(mm_isp_eb, "mm-isp-eb", &isp_clk.common.hw,
			   0x0, 0x1000, BIT(3), 0, 0);
...
static SPRD_SC_GATE_CLK_HW(mm_isp_ahb_eb, "mm-isp-ahb-eb", &mm_isp_eb.common.hw,
			   0x0, 0x1000, BIT(8), 0, 0);
static SPRD_SC_GATE_CLK_HW(mm_dvfs_eb, "mm-dvfs-eb", &mm_eb.common.hw,
			   0x0, 0x1000, BIT(9), 0, 0);
static SPRD_SC_GATE_CLK_HW(mm_fd_eb, "mm-fd-eb", &fd_clk.common.hw,
			   0x0, 0x1000, BIT(10), 0, 0);
static SPRD_SC_GATE_CLK_HW(mm_sensor2_en, "mm-sensor2-en",
			   &sensor2_clk.common.hw, 0x8, 0x1000, BIT(0),
			   CLK_SET_RATE_PARENT, 0);
static SPRD_SC_GATE_CLK_HW(mm_sensor1_en, "mm-sensor1-en",
			   &sensor1_clk.common.hw, 0x8, 0x1000, BIT(1),
			   CLK_SET_RATE_PARENT, 0);
static SPRD_SC_GATE_CLK_HW(mm_sensor0_en, "mm-sensor0-en",
			   &sensor0_clk.common.hw, 0x8, 0x1000, BIT(2),
			   CLK_SET_RATE_PARENT, 0);
...
static struct sprd_reset_map ums9230_mm_ahb_resets[] = {
	...
	[RESET_MM_AHB_CSI2DCAM_SW_SOFT_RST]	= { 0x0004, BIT(28), 0x1000 },
	[RESET_MM_AHB_AXI_REGU_SOFT_RST]	= { 0x0004, BIT(29), 0x1000 },
};
...
static struct sprd_reset_map ums9230_audcp_ahb_resets[] = {
