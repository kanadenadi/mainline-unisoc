...
static int panfrost_clk_init(struct panfrost_device *pfdev)
{
	...
	pfdev->mem_clock = devm_clk_get_optional(pfdev->dev, "mem");
	if (IS_ERR(pfdev->mem_clock)) {
		dev_err(pfdev->dev, "get mem_clock failed %ld\n",
	...
	if (pfdev->mem_clock) {
		rate = clk_get_rate(pfdev->mem_clock);
		dev_info(pfdev->dev, "mem_clock rate = %lu\n", rate);
