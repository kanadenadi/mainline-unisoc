...
static void  sprd_i2c_hw_set_clk(struct sprd_i2c_hw *i2c_dev, u32 freq)
{
	...
	case I2C_CLK_100K:
		...
		break;
	default:
		dev_err(i2c_dev->dev, "Invalid bus frequency!");
		break;
	}
}
...
static int sprd_i2c_hw_probe(struct platform_device *pdev)
{
	...
	i2c_dev = devm_kzalloc(&pdev->dev, sizeof(struct sprd_i2c_hw), GFP_KERNEL);
	if (!i2c_dev)
		return -ENOMEM;
...
static void sprd_i2c_hw_remove(struct platform_device *pdev)
{
